...
export default function AspectRatioSelector({ value, onChange, options: allowedOptions }: AspectRatioSelectorProps) {
    ...
    const options = allowedOptions 
        ? allOptions.filter(opt => allowedOptions.includes(opt.value))
        : allOptions;

    const selectedOption = options.find(opt => opt.value === value) || options[0] || allOptions[0];
