// app/login/page.tsx
/**
 * Login page with Google OAuth
 */

"use client"

import { useEffect } from 'react';
import { useRouter } from 'next/navigation';
import { useAuth } from '@/hooks/useAuth';
import { Sparkles, Loader2 } from 'lucide-react';
import Button from '@/components/common/Button';

export default function LoginPage() {
    const { login, isAuthenticated, isLoading } = useAuth();
    const router = useRouter();

    useEffect(() => {
        if (isAuthenticated) {
            router.push('/create-image');
        }
    }, [isAuthenticated, router]);

    if (isLoading) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-background">
                <Loader2 className="h-8 w-8 animate-spin text-primary" />
            </div>
        );
    }

    return (
        <div className="min-h-screen flex items-center justify-center bg-background p-4">
            <div className="w-full max-w-md">
                {/* Login Card */}
                <div className="bg-card border border-border rounded-2xl shadow-lg p-8">
                    {/* Logo/Header */}
                    <div className="text-center mb-8">
                        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-[#0F766E]/10 mb-4">
                            <Sparkles className="h-8 w-8 text-[#0F766E]" />
                        </div>
                        <h1 className="text-2xl font-bold text-foreground mb-2">
                            Trạm Sáng Tạo
                        </h1>
                        <p className="text-muted-foreground">
                            Tạo ảnh và Video AI nhanh chóng - tự động.
                        </p>
                    </div>

                    {/* Google Login Button */}
                    <Button
                        onClick={login}
                        className="w-full bg-[#0F766E] hover:bg-[#0D655E] text-white font-medium h-12 rounded-lg shadow-sm transition-all duration-200 flex items-center justify-center gap-3"
                    >
                        {/* Google Icon */}
                        <svg className="h-5 w-5" viewBox="0 0 24 24">
                            <path
                                fill="currentColor"
                                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                            />
                            <path
                                fill="currentColor"
                                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                            />
                            <path
                                fill="currentColor"
                                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                            />
                            <path
                                fill="currentColor"
                                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                            />
                        </svg>
                        Đăng nhập với Google
                    </Button>

                    {/* Promo Text */}
                    <div className="mt-6 p-4 rounded-lg bg-[#0F766E]/5 border border-[#0F766E]/20">
                        <p className="text-sm text-center text-foreground">
                            🎁 <span className="font-semibold">Nhận 1000 credits miễn phí</span>
                            <br />
                            <span className="text-muted-foreground">khi đăng ký lần đầu!</span>
                        </p>
                    </div>
                </div>

                {/* Footer */}
                <p className="text-center text-xs text-muted-foreground mt-6">
                    Công cụ tạo hình ảnh - video - studio AI miễn phí
                </p>
            </div>
        </div>
    );
}
